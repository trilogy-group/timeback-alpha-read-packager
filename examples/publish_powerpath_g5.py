...
import arpack                           # passage_html_div (renderer), assert_kid_facing_title (guard)
...
def _parse_mastery(v):
    ...
    try:
        ...
        raise SystemExit("--mastery-threshold must be an integer or one of none/off/0: %r" % v)
...
def main():
    ...
    ap.add_argument("--mastery-threshold", default="90",
                    help="powerpath-100 resource masteryThreshold (default 90; '0'/'none'/'off' disables the gate)")
    ap.add_argument("--lenient-titles", action="store_true",
                    help="downgrade the kid-facing-title guard from fail-closed to warn-only")
    a = ap.parse_args()
    _MT = _parse_mastery(a.mastery_threshold)
    ...
    for _scope, _objs in (("unit", plan["units"]), ("lesson", plan["lessons"])):
        for _o in _objs:
            try:
                arpack.assert_kid_facing_title(_o.get("title", ""))
            except ValueError as _e:
                _title_violations.append("%s %s: %s" % (_scope, _o.get("sourcedId", "?"), _e))
    if _title_violations:
        print("KID-FACING TITLE GUARD: %d violation(s)" % len(_title_violations))
        for _v in _title_violations[:20]:
            print("  ✗ " + _v)
        if not a.lenient_titles:
