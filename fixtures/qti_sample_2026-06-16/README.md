...
This folder contains sample `qti-sdk build` output for four choice items:

| Item ID | Type | Stimulus |
|---------|------|----------|
| `sample-mcq-no-stimulus` | MCQ (single-select) | No |
| `sample-mcq-with-stimulus` | MCQ (single-select) | Yes |
| `sample-msq-no-stimulus` | MSQ (multi-select) | No |
| `sample-msq-with-stimulus` | MSQ (multi-select) | Yes |
...
new_timeback_build_output/
  manifest.json
  items/
    sample-mcq-no-stimulus.xml
    sample-mcq-with-stimulus.xml
    sample-msq-no-stimulus.xml
    sample-msq-with-stimulus.xml
  stimuli/
...
- Source content is adapted from `examples/example_mcq.py`.
